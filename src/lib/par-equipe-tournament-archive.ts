...
export interface ParEquipeArchiveTeam { id?: string; name: string; teamLogo?: string; clubLogo?: string; club?: string; country?: string; coach?: string; roster: ParEquipeArchivePlayer[]; }
...
export function buildParEquipeArchiveTeams(args:{teams:Array<{id?:string;name:string;photo?:string;clubPhoto?:string;club?:string;country?:string;coach?:string;players?:Array<{id?:string;name:string;nationality?:string;playerNumber?:number;seedNumber?:number;photo?:string;maxRounds?:number;rounds?:number}>}>}):ParEquipeArchiveTeam[]{return args.teams.map(t=>({id:t.id,name:t.name,teamLogo:t.photo,clubLogo:t.clubPhoto,club:t.club,country:t.country,coach:t.coach,roster:(t.players||[]).map(p=>({id:p.id,name:p.name,nationality:p.nationality,playerNumber:p.playerNumber,seedNumber:p.seedNumber,photo:p.photo,rounds:p.rounds??p.maxRounds}))}));}
function archiveFromTournament(t:any){
 const bd=t?.bracket_data||{};const teams=(t.players||[]).map((team:any)=>({id:team.id,name:team.name,photo:team.teamLogo||team.photo,clubPhoto:team.clubLogo,club:team.club,country:team.nationality,coach:team.coachName || undefined,players:bd.teamRosters?.[team.id]||[]}));
