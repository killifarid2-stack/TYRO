...
export type AnimatableLayerProperty =
  ...
  | 'text' | 'imageSrc' | 'videoSrc' | 'binding' | 'effect' | 'frameStyle' | 'effectColor' | 'effectIntensity'
...
export function evaluateLayerAtTime(layerValue: DesignLayer, time: number): DesignLayer {
  ...
  keys.forEach(key => {
    ...
    // No keyframe at or before this time — an edit added later in the
    // timeline (e.g. a one-off correction at t=3) must not reach back and
    // change how the layer renders earlier in the animation. Hold the
    // layer's original base-pose value until the first authored keyframe,
    // instead of jumping forward to it.
    if (!before) { base[key] = baseValue; return; }
...
function baseLayers(animationId: DesignAnimationId): DesignLayer[] {
  ...
  return [
    ...
    layer(`${prefix}-flag`, 'Country Flag', 'image', { parentId: `${prefix}-info-group`, x: 290, y: 210, width: 140, height: 85, zIndex: 22, binding: (animationId === 'winner' || animationId === 'match-result') ? 'winnerFlag' : 'flag' }),
