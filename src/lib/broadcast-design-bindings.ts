...
function inferBinding(layer: DesignLayer): BindingKey | undefined {
  const key = `${layer.id} ${layer.name}`.toLowerCase();
  if (/(winner|champion|result).*flag|flag.*(winner|champion|result)/.test(key)) return 'winnerFlag';
  if (/(^|[-_: ])(blue|chung).*flag|flag.*(blue|chung)/.test(key)) return 'flag';
  if (/(^|[-_: ])(red|hong).*flag|flag.*(red|hong)/.test(key)) return 'flag';
  return undefined;
}
...
export function resolveDesignBinding(layer: DesignLayer, state: any): DesignLayer {
  ...
  const inferred = !conditional.binding && conditional.type === 'image' ? inferBinding(conditional) : undefined;
  if (inferred) return resolveDesignBinding({ ...conditional, binding: inferred }, state);
