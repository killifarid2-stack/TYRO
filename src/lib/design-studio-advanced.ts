...
export function evaluateFrameByFrame(design:BroadcastDesign, fps: 24|25|30|50|60 = 60){ return designTimelineFrameTimes(design,fps).map(time=>({time,layers:design.layers.map(l=>evaluateLayerAtTime(l,time))})); }
