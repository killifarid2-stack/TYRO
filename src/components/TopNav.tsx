...
const HOME_ITEMS = [
  { path: '/operator', label: 'مباراة فردية', en: 'Individual Match', fr: 'Match individuel', icon: Swords },
  { path: '/operator', label: 'الحكم الرئيسي', en: 'Operator', fr: 'Opérateur', icon: Gavel },
  { path: '/scoreboard', label: 'شاشة الجمهور', en: 'Audience', fr: 'Public', icon: Monitor },
  { path: '/judge', label: 'الحكام', en: 'Judge', fr: 'Juge', icon: Users },
  { path: '/tournament', label: 'البطولة', en: 'Tournament', fr: 'Tournoi', icon: Trophy },
  { path: '/admin', label: 'الإدارة', en: 'Admin', fr: 'Administration', icon: Settings },
  { path: '/', label: 'Par Équipe', en: 'Par Équipe', fr: 'Par Équipe', icon: Swords, state: { openParEquipe: true, scrollTo: 'par-equipe-archive' } },
] as const;

const OTHER_ITEMS = [
  { path: '/club-points', label: 'ترتيب الأندية', en: 'Club Ranking', fr: 'Classement des clubs', icon: ShieldAlert },
  { path: '/control-room', label: 'غرفة التحكم', en: 'Control Room', fr: 'Salle de contrôle', icon: Radio },
  { path: '/tournament-control', label: 'مركز الشجرة', en: 'Bracket Control', fr: 'Contrôle du tableau', icon: GitBranch },
  ...
  { path: '/tournament-finalization', label: 'إنهاء البطولة', en: 'Tournament Finalization', fr: 'Finalisation du tournoi', icon: ListChecks },
  { path: '/final-command', label: 'مركز الأوامر النهائي', en: 'Final Command Center', fr: 'Centre de commande final', icon: ClipboardList },
  { path: '/qa-test-center', label: 'مركز الاختبار', en: 'QA / Test Center', fr: 'Centre QA / Tests', icon: Activity },
  { path: '/flags', label: 'الأعلام والدول', en: 'Flags & Countries', fr: 'Drapeaux & Pays', icon: Flag },
  { path: '/install', label: 'التثبيت', en: 'Install', fr: 'Installation', icon: Settings2 },
] as const;
  ...
  return (
    ...
    >
      ...
      <button
        ...
        title={location.pathname === '/tournament' ? (lang === 'ar' ? 'حفظ البطولة والفئة' : lang === 'fr' ? 'Enregistrer tournoi / catégorie' : 'Save Tournament / Category') : (lang === 'ar' ? 'حفظ' : lang === 'fr' ? 'Enregistrer' : 'Save')}
      >
      ...
      <div className="app-no-drag relative shrink-0" ref={menuRef}>
        <button onClick={() => setMenuOpen(v => !v)} className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold shrink-0 transition-colors ${menuOpen ? 'bg-[hsl(var(--primary))]/20 text-[hsl(var(--primary))]' : 'bg-[hsl(var(--secondary))]/70 text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))]'}`} aria-expanded={menuOpen} title={lang === 'ar' ? 'باقي النوافذ' : 'Other windows'}>
          <Menu size={13} /><span>{lang === 'ar' ? 'القائمة' : lang === 'fr' ? 'MENU' : 'MENU'}</span><ChevronDown size={12} className={menuOpen ? 'rotate-180' : ''} />
        </button>
        {menuOpen && createPortal(
          <div data-wab-top-menu className="fixed z-[100000] w-[min(320px,calc(100vw-20px))] max-h-[72vh] overflow-y-auto overscroll-contain rounded-xl border border-white/10 bg-[#0a0c12] p-1.5 shadow-[0_24px_70px_rgba(0,0,0,.65)]" style={{ left: menuPos.left, top: menuPos.top }}>
            {extras.map(item => { const Icon = item.icon; const active = location.pathname === item.path; return <button key={item.path} onClick={() => go(item.path)} className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-xs font-semibold text-start transition-colors ${active ? 'bg-[hsl(var(--primary))]/20 text-[hsl(var(--primary))]' : 'text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] hover:bg-white/5'}`}><Icon size={14} /><span>{lang === 'ar' ? item.label : lang === 'fr' ? item.fr : item.en}</span></button>; })}
          </div>, document.body
        )}
      </div>
      {state.matNumber ? <span className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-[hsl(var(--gold))]/15 text-[hsl(var(--gold))] text-xs font-bold shrink-0"><MapPin size={12} /> {lang === 'ar' ? 'بساط' : lang === 'fr' ? 'AIRE' : 'MAT'} {String(state.matNumber).padStart(2, '0')}</span> : null}
      <div className="app-no-drag ms-auto flex items-center gap-1.5 shrink-0"><PublicDisplayControl /><BroadcastControl /><RoleSwitcher /><ConnectionBadge compact /><select aria-label="Language" value={lang} onChange={e => setLang(e.target.value as 'en'|'ar'|'fr')} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-[hsl(var(--gold))]/10 text-[hsl(var(--gold))] text-xs font-semibold shrink-0 border border-[hsl(var(--gold))]/20 outline-none"><option value="ar">العربية</option><option value="en">English</option><option value="fr">Français</option></select></div>
    </nav>
  );
}
