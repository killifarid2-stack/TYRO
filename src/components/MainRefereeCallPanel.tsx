...
export default function MainRefereeCallPanel({ state, dispatch, modal = false, onClose, openControlsOnMount = false, openTeamControlsOnMount = false }: Props) {
  ...
  return (
    <section dir="ltr" className={`${modal ? 'fixed inset-0 z-[260] m-0 min-h-screen w-full overflow-y-auto rounded-none p-3 md:p-5 lg:p-7' : fullScreen ? 'fixed inset-0 z-[480] m-0 min-h-screen w-full overflow-y-auto rounded-none p-4 md:p-6 lg:p-8' : 'mt-3 w-full rounded-2xl p-4'} relative overflow-hidden border-2 border-[hsl(var(--gold))]/30 bg-[radial-gradient(circle_at_10%_0%,rgba(242,193,78,.10),transparent_28%),radial-gradient(circle_at_92%_8%,rgba(51,162,255,.08),transparent_25%),linear-gradient(145deg,#090c12,#05070b)] shadow-[0_0_70px_rgba(242,193,78,.10)]`}>
      ...
      {/* PLAYERS/MATCH/ROUND/TIME/STATUS/WEIGHT summary grid removed per
          request — this row duplicated info already visible elsewhere and
          left a mostly-empty panel below the actual controls. The two
          controls buttons (Player Call / Team Call) stay in the header row
          ...
          <div className="max-h-[88vh] w-full max-w-3xl overflow-y-auto rounded-2xl border-2 border-[#33a2ff]/40 bg-[radial-gradient(circle_at_15%_0%,rgba(51,162,255,.08),transparent_30%),#06080c] p-4 md:p-6 shadow-[0_0_100px_rgba(51,162,255,.12)]" onMouseDown={(e) => e.stopPropagation()}>
              ...
              <div className="mb-3 rounded-xl border border-[#ffb020]/30 bg-[#ffb020]/[.035] p-3">
                <div className="mb-2 flex items-center justify-between gap-2">
                  ...
                  {state.substitutionAnimation && (
                    <span className="text-[8px] font-black uppercase tracking-[.14em] text-[#39ff6a]">{T('ANIMATION ACTIVE','الأنيميشن نشط','ANIMATION ACTIVE')}</span>
                  )}
                </div>
            ...
            <div className="mb-3 rounded-xl border border-[#33a2ff]/25 bg-[#33a2ff]/5 p-3">
              ...
              <div className="mb-3 flex flex-wrap gap-1.5">
                <button type="button" onClick={savePlayerCall} className="rounded-md border px-2.5 py-1.5 text-[9px] font-black" style={{ borderColor:'#f2c14e', color:'#f2c14e', background:'#f2c14e12' }}><Save size={11} className="inline me-1"/>{T('SAVE','حفظ','ENREGISTRER')}</button>
                <button type="button" onClick={resetPlayerCall} className="rounded-md border border-white/20 px-2.5 py-1.5 text-[9px] font-black text-white/70"><RotateCcw size={11} className="inline me-1"/>{T('RESET','إعادة ضبط','RÉINITIALISER')}</button>
                <button type="button" onClick={() => startPlayerPreview('intro')} className="rounded-md border border-[#33a2ff]/60 px-2.5 py-1.5 text-[9px] font-black text-[#33a2ff]"><Eye size={11} className="inline me-1"/>{T('PREVIEW','معاينة','APERÇU')}</button>
                <button type="button" onClick={() => dispatch({ type: 'START_PLAYER_CALL_SEQUENCE' })} disabled={busy || playerBusy} className="rounded-md border-2 border-[#39ff6a]/70 bg-[#39ff6a]/10 px-2.5 py-1.5 text-[9px] font-black text-[#39ff6a] disabled:opacity-30"><Play size={11} className="inline me-1"/>{T('PLAY','تشغيل','LECTURE')}</button>
                <button type="button" disabled={playerStatuses.chung !== 'ready' || playerStatuses.hong !== 'called'} onClick={() => confirmPlayerReadyAndGoLive('hong')} className="rounded-md border border-[#39ff6a]/60 px-2.5 py-1.5 text-[9px] font-black text-[#39ff6a] disabled:opacity-30"><ShieldCheck size={11} className="inline me-1"/>{T('PLAYER READY / CONFIRM','اللاعب جاهز / تأكيد','JOUEUR PRÊT / CONFIRMER')}</button>
                <button type="button" onClick={() => dispatch({ type: 'REPLAY_SINGLE_PLAYER_CALL' })} disabled={!playerCall || playerCall.status === 'calling'} className="rounded-md border border-white/20 px-2.5 py-1.5 text-[9px] font-black text-white/70 disabled:opacity-30"><RotateCcw size={11} className="inline me-1"/>{T('REPLAY','إعادة','REJOUER')}</button>
                    ...
                    <div key={side} className="rounded-lg border p-2.5" style={{ borderColor: `${accent}55`, background: `${accent}08` }}>
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex flex-wrap justify-end gap-1.5">
                          ...
                          {status === 'called' && <button type="button" onClick={() => confirmPlayerReadyAndGoLive(side)} className="rounded-full border border-[#39ff6a]/60 px-2.5 py-1 text-[9px] font-black text-[#39ff6a]"><CheckCircle2 size={11} className="inline me-1" /> {T('CONFIRM','تأكيد','CONFIRMER')}</button>}
                          {(status === 'called' || status === 'ready') && <button type="button" disabled={playerBusy} onClick={() => dispatch({ type: 'REPLAY_SINGLE_PLAYER_CALL' })} className="rounded-full border border-white/20 px-2.5 py-1 text-[9px] font-black text-white/70 disabled:opacity-30"><RotateCcw size={11} className="inline me-1" /> {T('REPLAY','إعادة','REJOUER')}</button>}
                          {(status === 'called' || status === 'ready') && selected?.name && <button type="button" disabled={playerBusy} onClick={() => dispatch({ type: 'RECALL_SINGLE_PLAYER_CALL', side })} className="rounded-full border border-white/20 px-2.5 py-1 text-[9px] font-black text-white/70 disabled:opacity-30">{T('RECALL','استدعاء مجدد','RAPPELER')}</button>}
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                }} className="rounded-full border-2 border-[#ffd866] bg-[#ffd866]/10 px-3 py-1.5 text-[10px] font-black text-[#ffd866] disabled:opacity-30"><Eye size={12} className="inline me-1" /> {T('TV / GO LIVE','البث / مباشر','TV / DIRECT')}</button>
                {playerCall && <button type="button" onClick={() => dispatch({ type: 'RECALL_SINGLE_PLAYER_CALL', side: playerCall.side })} className="rounded-full border border-white/20 px-3 py-1.5 text-[10px] font-black text-white/70">{T('RECALL PLAYER','استدعاء اللاعب مجددًا','RAPPELER LE JOUEUR')}</button>}
                {playerBusy && <button type="button" onClick={() => dispatch({ type: 'STOP_BROADCAST_ANIMATION' })} className="rounded-full border-2 border-red-500/60 bg-red-500/10 px-3 py-1.5 text-[10px] font-black text-red-300"><X size={12} className="inline me-1" /> {T('STOP PLAYER CALL','إيقاف استدعاء اللاعب','ARRÊTER L’APPEL')}</button>}
              </div>
            </div>
          </div>
        </div>
            ...
            {(() => {
              ...
              return <>
                  ...
                  {(['chung', 'hong'] as PlayerColor[]).map((side) => {
                    ...
                    return <div key={side} className="rounded-xl border p-3" style={{ borderColor: `${accent}55`, background: `${accent}08` }}>
                      <div className="mb-2 flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          {status === 'calling' && <span className="rounded-full border border-[#39ff6a]/40 bg-[#39ff6a]/10 px-3 py-1.5 text-[9px] font-black text-[#39ff6a]">{T('CALLING','جاري الاستدعاء','APPEL EN COURS')}</span>}
                ...
                <div className="mb-3 rounded-xl border border-[#f2c14e]/20 bg-black/35 p-3">
                  <div className="mb-2 text-[9px] font-black uppercase tracking-[.16em] text-white/40">{T('TEAM CALL COMMAND BAR','شريط أوامر استدعاء الفرق','BARRE DE COMMANDE APPEL ÉQUIPES')}</div>
                  <div className="flex flex-wrap gap-2">
                    <button type="button" onClick={()=>dispatch({type:'START_TEAM_CALL_SEQUENCE'})} disabled={busy || playerBusy || state.config?.competitionMode !== 'par_equipe'} className="rounded-full border-2 border-[#ffd866] bg-[#ffd866] px-4 py-2 text-[10px] font-black text-black disabled:opacity-30">{T('START TEAM CALL','بدء استدعاء الفرق','DÉMARRER L’APPEL DES ÉQUIPES')}</button>
                    {(busy || playerBusy) && <button type="button" onClick={()=>dispatch({type:'STOP_BROADCAST_ANIMATION'})} title="Clears a stuck player/team call so Team Call buttons respond again" className="rounded-full border-2 border-red-500/60 bg-red-500/10 px-3 py-2 text-[10px] font-black text-red-300"><X size={12} className="inline me-1" /> {T('RESET CALL STATE','إعادة ضبط حالة الاستدعاء','RÉINITIALISER L’ÉTAT')}</button>}
                    ...
                    <button type="button" onClick={()=>dispatch({type:'SET_CALL_CONTROL_WAITING'})} className="rounded-full border border-white/20 px-3 py-2 text-[10px] font-black text-white/70">{T('AWAITING / HOLD','انتظار / تعليق','EN ATTENTE / PAUSE')}</button>
                    <button type="button" onClick={()=>dispatch({type:'CALL_TEAM', side:'chung'})} disabled={!!state.callAnimation || busy || playerBusy || state.config?.competitionMode !== 'par_equipe'} className="rounded-full border-2 border-[#33a2ff]/70 bg-[#33a2ff]/10 px-3 py-2 text-[10px] font-black text-[#33a2ff] disabled:opacity-30">{T('BLUE TEAM','الفريق الأزرق','ÉQUIPE BLEUE')}</button>
                    <button type="button" onClick={()=>dispatch({type:'CALL_TEAM', side:'hong'})} disabled={!!state.callAnimation || busy || playerBusy || state.config?.competitionMode !== 'par_equipe'} className="rounded-full border-2 border-[#ff2b39]/70 bg-[#ff2b39]/10 px-3 py-2 text-[10px] font-black text-[#ff5360] disabled:opacity-30">{T('RED TEAM','الفريق الأحمر','ÉQUIPE ROUGE')}</button>
