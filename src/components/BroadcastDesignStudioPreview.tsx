...
export default function BroadcastDesignStudioPreview({
  ...
  if (['best-player-match','team-match-mvp','tournament-mvp','best-team','best-club','best-referee','fair-play','top-scorer','top-hitter','podium','awards'].includes(animationId)) {
...
function buildPreviewState(source: MatchState, animationId: DesignAnimationId, previewStage = 'main'): MatchState {
  const base = createInitialMatchState({ ...source.config, competitionMode: animationId === 'team-call' || animationId === 'player-change' ? 'par_equipe' : 'knockout' }, source.displayConfig, true);
  ...
  switch(animationId){
    ...
    case 'woose-girok': { const girokPhase = (['summons','countdown','voting','result'] as const).includes(previewStage as any) ? previewStage as 'summons'|'countdown'|'voting'|'result' : 'summons'; state.status='rest'; state.roundTieReview={phase:girokPhase,ts:now,countdownStep:3,votes:{left:'chung',center:'chung',right:'hong'},judgeNames:{left:'JUDGE 1',center:'CENTER REFEREE',right:'JUDGE 3'},judgePhotos:{}}; state.animationController={state:'IDLE',activeAnimation:'WOO_SE_GIROK' as any,animationId:`studio-girok-${animationId}`,instanceId:`studio-girok-${animationId}`,startedAt:now}; break; }
    ...
    case 'round-call': state.status='rest'; state.teamMode='rotation'; state.roundCallPreview={round:2,ts:now,chung:{name:'BLUE PLAYER 2',nationality:'MAR',playerNumber:2,seedNumber:2},hong:{name:'RED PLAYER 2',nationality:'MAR',playerNumber:2,seedNumber:2}}; state.animationController={state:'IDLE',activeAnimation:'ROUND_CALL' as any,animationId:`studio-round-call-${animationId}`,instanceId:`studio-round-call-${animationId}`,startedAt:now}; break;
