...
import { AGE_GROUPS, GENDERS, COMPETITION_MODES, getWeightCategories } from '@/lib/tkd-data';
import { logAudit } from '@/lib/audit-log';
import { saveTournamentLocal } from '@/lib/tournament-local';
import { saveMatchLocal } from '@/lib/match-local';
...
/** Nationality field. This used to be a plain <select> built from the 109
 *  hard-coded entries in TKD_COUNTRIES, which meant a competitor from any
 *  other country simply could not be entered. It is now the searchable
 *  CountryPicker over the full bundled country list (search by 3-letter
 *  code, ISO-2 or name in AR/EN/FR), with the real flag shown inline. */
const CountryField = ({ label, value, onChange, placeholder }: {
  label: string; value: string; onChange: (v: string) => void; placeholder?: string;
}) => (
  <div>
    <label className="text-xs text-muted-foreground font-semibold block mb-1">{label}</label>
    <CountryPicker value={value} onChange={onChange} placeholder={placeholder} />
  </div>
);
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    ...
    if (cont.matchId) {
      // The category library match id becomes the durable match identity for
      // this continuation. It is carried through SET_MATCH_INFO below.
      setCategoryMatchId(String(cont.matchId));
    }
  ...
  const [categoryMatchId, setCategoryMatchId] = useState<string | null>(null);
  ...
  const handleSave = async () => {
    ...
    dispatch({
      ...
      tournamentId,
      ...(categoryMatchId ? { bracketMatchId: categoryMatchId } : {}),
    });

    // Save to database. A category continuation already owns a durable
    // match row; never INSERT a second waiting row for it. Update the exact
    // id locally/cloud instead, so the same match transitions WAITING → LIVE
    // → COMPLETED without ever becoming playable twice.
    try {
      if (categoryMatchId) {
        const categoryRecord: any = {
          id: categoryMatchId,
          ...
          hong_name: hongName,
          chung_nationality: chungNat || null,
          hong_nationality: hongNat || null,
          status: 'waiting',
          winner: null,
        ...
        saveMatchLocal(categoryRecord);
        // Local category ids are intentionally non-UUID. Cloud rows are UUIDs,
        // so only send a cloud update when this is a real remote match id.
        if (/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(String(categoryMatchId))) {
          await supabase.from('matches').update({
            competition_name: compName, match_number: assignedNum, weight_category: weightCat,
            gender, age_group: ageGroup, match_stage: matchStage || null,
            chung_name: chungName, hong_name: hongName,
            chung_nationality: chungNat || '', hong_nationality: hongNat || '',
            status: 'waiting', tournament_id: tournamentId,
            competition_mode: config.competitionMode,
          } as any).eq('id', categoryMatchId);
        }
      } else {
      ...
      <div className="p-4 max-w-2xl mx-auto space-y-4">
        ...
        <div className="panel p-4">
          <h2 className="font-display text-sm font-bold text-foreground mb-2">
            {lang === 'ar' ? 'ربط بالبطولة' : lang === 'fr' ? 'Lier au tournoi' : 'Attach to Tournament'}
          </h2>
          <TournamentPicker
            onPick={(tr) => {
          ...
          {pickedTournament && (
            <div className="mt-2 flex items-center justify-between text-[11px] bg-primary/10 text-primary rounded-lg px-3 py-1.5">
              <span>{lang === 'ar' ? 'مرتبط بـ' : lang === 'fr' ? 'Lié à' : 'Linked to'}: <b>{pickedTournament.name}</b></span>
        ...
        <div className="panel p-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2 p-3 rounded-lg border border-hong/30 bg-hong/5 order-1">
              ...
              <CountryField label={t('nationality')} value={hongNat} onChange={setHongNat} placeholder={t('selectPlaceholder')} />
            ...
            <div className="space-y-2 p-3 rounded-lg border border-chung/30 bg-chung/5 order-2">
              ...
              <CountryField label={t('nationality')} value={chungNat} onChange={setChungNat} placeholder={t('selectPlaceholder')} />
        ...
        {/* Flag appearance — sits directly under the two nationality fields
            so the operator can see, before the match starts, exactly how
            the flags they just picked will be drawn on the public screen.
            It edits the one global setting (also reachable at /flags), and
            the change reaches the public display window immediately. */}
        <div className="panel p-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-display text-sm font-bold text-foreground">
              {lang === 'ar' ? 'شكل الأعلام' : lang === 'fr' ? 'Apparence des drapeaux' : 'Flag appearance'}
            </h2>
            <button type="button" onClick={() => navigate('/flags')}
              className="text-[11px] px-2 py-1 rounded-lg bg-secondary border border-border text-muted-foreground hover:text-foreground transition">
              {lang === 'ar' ? 'كل الدول والاختبار' : lang === 'fr' ? 'Tous les pays & test' : 'All countries & test'}
            </button>
          </div>
          <FlagStyleControls previewCode={hongNat || chungNat || 'MAR'} />
        </div>
        ...
        <div className="panel p-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <InputField label={lang === 'ar' ? 'مدة أنيميشين تغيير اللاعب (ثواني)' : lang === 'fr' ? 'Durée de l’animation de changement (s)' : 'Player-change animation duration (sec)'} value={config.parEquipeTagSeconds ?? 5} onChange={(v: number) => setConfig({...config, parEquipeTagSeconds: v})} type="number" />
          </div>
        ...
        {/* Design controls — milestone 1: Team Call center frame only.
            Moves/resizes/recolors the big center trophy frame on the Team
            Call broadcast. Leaving all fields at their defaults (0, 0, 1,
            empty) reproduces the exact original look — nothing changes
            until these are touched. More elements/animations get their own
            panel here over time; this is the first one. */}
        <div className="panel p-4">
          <h2 className="font-display text-sm font-bold text-foreground mb-1">{lang === 'ar' ? 'تصميم — إطار استدعاء الفرق (تجريبي)' : lang === 'fr' ? 'Design — cadre central d’appel des équipes (bêta)' : 'Design — Team Call Center Frame (Beta)'}</h2>
          <p className="text-[11px] text-muted-foreground mb-3">{lang === 'ar' ? 'حرك/كبر/صغر/لون إطار الكأس الكبير وسط شاشة استدعاء الفرق.' : lang === 'fr' ? 'Déplacer, redimensionner et recolorer le grand cadre du trophée au centre.' : 'Move, resize, and recolor the big center trophy frame on the Team Call screen.'}</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <InputField label={lang === 'ar' ? 'إزاحة أفقية (px)' : lang === 'fr' ? 'Décalage X (px)' : 'Offset X (px)'} value={config.teamCallFrameOffsetX ?? 0} onChange={(v: number) => setConfig({...config, teamCallFrameOffsetX: v})} type="number" />
            <InputField label={lang === 'ar' ? 'إزاحة عمودية (px)' : lang === 'fr' ? 'Décalage Y (px)' : 'Offset Y (px)'} value={config.teamCallFrameOffsetY ?? 0} onChange={(v: number) => setConfig({...config, teamCallFrameOffsetY: v})} type="number" />
            <InputField label={lang === 'ar' ? 'الحجم (1 = عادي)' : lang === 'fr' ? 'Échelle (1 = normal)' : 'Scale (1 = normal)'} value={config.teamCallFrameScale ?? 1} onChange={(v: number) => setConfig({...config, teamCallFrameScale: v})} type="number" />
            <InputField label={lang === 'ar' ? 'لون الإطار (اختياري)' : lang === 'fr' ? 'Couleur de bordure (optionnel)' : 'Border color (optional)'} value={config.teamCallFrameBorderColor ?? ''} onChange={(v: string) => setConfig({...config, teamCallFrameBorderColor: v || undefined})} type="text" />
            <InputField label={lang === 'ar' ? 'لون عنوان الاستدعاء (اختياري)' : lang === 'fr' ? 'Couleur du titre d’appel (optionnel)' : 'Call title color (optional)'} value={config.teamCallTitleColor ?? ''} onChange={(v: string) => setConfig({...config, teamCallTitleColor: v || undefined})} type="text" />
          </div>
          <label className="mt-3 flex items-center gap-2 p-2 rounded-lg border border-border bg-secondary/40 cursor-pointer hover:bg-secondary/70 transition w-fit">
            <input type="checkbox" checked={config.teamCallFrameFrozen === true}
              onChange={e => setConfig({ ...config, teamCallFrameFrozen: e.target.checked })}
              className="w-4 h-4 accent-primary" />
            <span className="text-xs font-semibold text-foreground">{lang === 'ar' ? 'تجميد أنيميشين الإطار' : lang === 'fr' ? 'Figer l’animation du cadre' : 'Freeze frame animation'}</span>
          </label>
        </div>
