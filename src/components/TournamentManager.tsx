...
import { AGE_GROUPS, GENDERS, COMPETITION_MODES, CLUB_POINTS, getWeightCategories, suggestAgeGroup, formatFreeWeight } from '@/lib/tkd-data';
...
export default function TournamentManager() {
      ...
      try {
        ...
        if (!matchError) {
          for (const row of cloudMatches || []) {
            saveMatchLocal({
              ...(row as any), id: row.id, match_id: row.id, competition_name: row.competition_name || 'Tournament',
  ...
  const loadTournament = async (t: SavedTournament) => {
    ...
    setTeamCoaches((t.bracket_data as any)?.teamCoaches ?? {});
    ...
    if (isLocalTournamentId(t.id)) {
      setPlayers((Array.isArray(localPlayers) ? localPlayers : []).map((p:any) => ({ ...p, coachName: (t.bracket_data as any)?.teamCoaches?.[p.id] ?? p.coachName })));
    } else {
      try {
        ...
        if (rows && rows.length > 0) {
          setPlayers(rows.map(p => ({
            ...
            coachName: (t.bracket_data as any)?.teamCoaches?.[p.id] || undefined,
          })));
        } else {
          setPlayers((Array.isArray(localPlayers) ? localPlayers : []).map((p:any) => ({ ...p, coachName: (t.bracket_data as any)?.teamCoaches?.[p.id] ?? p.coachName })));
        }
  ...
  const handleAutoFillWeight = (ctx:{tournamentName:string; gender:'male'|'female'; ageGroup:string; weight:string; record?:any}) => {
    ...
    toast.success(lang === 'ar' ? `✓ تم ملء معلومات ${ctx.weight} تلقائياً — أضف اللاعبين الآن` : lang === 'fr' ? `✓ Informations ${ctx.weight} chargées — ajoutez maintenant les joueurs` : `✓ ${ctx.weight} information loaded — add players now`);
  };
  ...
  const handleSaveTournament = async (opts?: { bracketOverride?: BracketMatchDisplay[] | null; leagueOverride?: LeagueMatch[] | null; nameOverride?: string }) => {
    ...
    const bracketData = {
      ...
      teamCoaches: mode === 'par_equipe' ? Object.fromEntries(players.filter((p:any) => p.coachName).map((p:any) => [p.id, p.coachName])) : undefined,
  ...
  const addPlayer = async () => {
    ...
    const player: Player = {
      ...
      coachName: isParEquipe ? (newCoachName.trim() || undefined) : undefined,
    };
    ...
    setNewName(''); setNewNat(''); setNewClub(''); setNewCoachName(''); setNewPlayerNumber(''); setNewSeedNumber(''); setNewPhoto(''); setNewTeamLogo('');
  };
  ...
  const handleStartMatch = (p1: Player, p2: Player, matchNum: number, round?: number, totalRoundsForStage?: number, bracketMatchId?: string, matNumber?: number) => {
    if (isParEquipe) {
      ...
      dispatch({
        ...
        coachNames: { chung: (p1 as any).coachName, hong: (p2 as any).coachName },
      ...
      <div className="p-4 max-w-5xl mx-auto space-y-4">
        ...
        {saveOkNotice && (
          <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
            <div className="w-full max-w-md rounded-2xl border-2 border-emerald-400/35 bg-[#07110d] p-6 text-center shadow-[0_30px_100px_rgba(0,0,0,.75)]">
              <div className="text-lg font-black text-emerald-300">✓ {lang === 'ar' ? 'تم حفظ البطولة' : lang === 'fr' ? 'TOURNOI ENREGISTRÉ' : 'TOURNAMENT SAVED'}</div>
        ...
        <div className="panel p-4">
          ...
          <div className="flex gap-2 mb-3">
            ...
            {/* Was a bespoke dropdown over the 109 hard-coded TKD_COUNTRIES
                entries, matching on the raw code only. Now the shared
                CountryPicker: full country list, searchable by code or by
                name in Arabic/English/French, with the real flag inline. */}
            <div className={`w-28 ${tournamentHasStarted ? 'pointer-events-none opacity-60' : ''}`}>
              <CountryPicker value={newNat} onChange={setNewNat} placeholder={tr('countryLabel')} />
            </div>
            ...
            {isParEquipe && (
              <input disabled={tournamentHasStarted} value={newCoachName} onChange={e => setNewCoachName(e.target.value)} placeholder="Coach / مدرب" title="اسم المدرب"
                className="w-28 px-3 py-2 rounded-lg bg-[hsl(var(--secondary))] border border-[hsl(var(--border))] text-[hsl(var(--foreground))] text-sm" />
            )}
            {!isParEquipe && (
              <>
            ...
            {visiblePlayers.map((p, i) => (
              <div key={p.id} className="flex items-center justify-between px-3 py-2 rounded-lg bg-[hsl(var(--secondary))]/50 text-sm">
                ...
                <div className="flex items-center gap-2">
                  {isParEquipe && (
                    <input type="text" value={(p as any).coachName ?? ''} onChange={e => {
                      const coachName = e.target.value;
                      setPlayers(prev => prev.map(pl => pl.id === p.id ? { ...pl, coachName: coachName.trim() || undefined } : pl));
                      setTeamCoaches(prev => ({ ...prev, [p.id]: coachName.trim() }));
                    }} placeholder="Coach" title="اسم المدرب"
                      className="w-24 px-2 py-1 rounded bg-[hsl(var(--background))] border border-[hsl(var(--border))] text-[hsl(var(--foreground))] text-[10px]" />
                  )}
                  {!isParEquipe && (
                    <>
