  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col items-center text-center p-4 md:p-5 shrink-0">
        ...
        {(isCalled ? callAnim.coachName : undefined) || state.coachNames?.[side] ? (
          <div className="mt-2 px-3 py-1 rounded-md" style={{ border: `1px solid ${theme.accent}66`, background: 'rgba(0,0,0,.28)' }}>
            <span className="text-[8px] tracking-[.16em] text-white/40">COACH</span>
            <span className="ml-2 font-display font-black text-white/85 text-[11px]">{(isCalled ? callAnim.coachName : undefined) || state.coachNames?.[side]}</span>
          </div>
        ) : null}
      </div>
      ...
      {roster && roster.length > 0 && (
        <div className="tco-roster flex-1 min-h-0 overflow-y-auto px-3 pb-3">
          <div className="tco-roster-head" style={{ color: `${theme.accent}` }}>
            <span>SEED</span><span>PLAYER</span><span>ATHLETE / اللاعب</span><span>PHOTO</span><span>NAT</span>
          </div>
          {roster.map((p, i) => {
            ...
            return (
              ...
              >
                <span className="tco-pnum">{p.seedNumber ?? '—'}</span>
...
export default function TeamCallOverlay({ state, callAnim, cc }: Props) {
  return (
    <div dir="ltr" className="flex-1 min-h-0 flex flex-col relative">
      <div className="shrink-0 px-3 md:px-5 pb-2 grid grid-cols-3 md:grid-cols-6 gap-1.5">
        {[
          ['TOURNAMENT', state.competitionName || 'TAEKWONDO CHAMPIONSHIP'],
          ['WEIGHT', state.weightCategory || '—'],
          ['MAT', state.matNumber ? `MAT ${String(state.matNumber).padStart(2,'0')}` : 'MAT 01'],
          ['MATCH', state.matchNumber ? String(state.matchNumber).padStart(3,'0') : '---'],
          ['ROUND', `${state.currentRound || 1}/${state.config?.rounds || 3}`],
          ['AGE / GENDER', `${state.ageGroup || '—'} / ${state.gender === 'female' ? 'WOMEN' : state.gender === 'male' ? 'MEN' : (state.gender || '—')}`],
        ].map(([label,value]) => <div key={label} className="min-w-0 rounded-lg px-2 py-1.5 text-center" style={{ border:'1px solid rgba(255,216,102,.22)', background:'rgba(0,0,0,.28)' }}><div className="text-[7px] tracking-[.12em] text-white/35">{label}</div><div className="truncate font-display font-black text-white/85" style={{fontSize:10}}>{value}</div></div>)}
      </div>
      <div className="flex-1 min-h-0 grid grid-cols-[minmax(0,1fr)_minmax(90px,.4fr)_minmax(0,1fr)] items-stretch gap-2 md:gap-5 lg:gap-8 py-2 relative">
      ...
      <style>{`
        ...
        .tco-roster-head, .tco-roster-row { display:grid; grid-template-columns: 48px 60px minmax(0,1fr) 34px 44px; align-items:center; gap:6px; }
