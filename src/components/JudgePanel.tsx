...
export default function JudgePanel({ judgeId, judgeName }: { judgeId: string; judgeName: string }) {
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      // Realtime is protected by the Supabase `authenticated` policy. The
      // judge UI has no password-login screen, so make sure its anonymous
      // authenticated session exists BEFORE subscribing to either channel.
      ...
      channel = supabase.channel('judge-votes')
        ...
        .on('broadcast', { event: 'score-vote-result' }, (payload) => {
          ...
          if (status === 'approved') setVoteNotification(`✓ ${SCORE_LABELS[type]} → ${player.toUpperCase()} approved`);
          else setVoteNotification(`✗ ${SCORE_LABELS[type]} → ${player.toUpperCase()} rejected`);
      ...
      stateChannel = supabase.channel('match-sync')
        .on('broadcast', { event: 'match-state' }, (payload) => {
          if (payload.payload) dispatch({ type: 'SET_STATE', state: payload.payload });
        })
        .subscribe((status) => {
          if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT' || status === 'CLOSED') setConnStatus('reconnecting');
        });
    };
