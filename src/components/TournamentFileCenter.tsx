...
function WeightCard({
  ...
  // WeightCard's own `t` prop is the match/tournament record (shadowing
  // useI18n()'s `t`), so only `lang` is pulled here, and this component
  // gets its own copy of the same inline trilingual-label helper used by
  // its parent TournamentFileCenter — it can't see that outer T() since
  // this is a separate function component.
  const { lang } = useI18n();
  const T = (en: string, ar: string, fr = en) => lang === 'ar' ? ar : lang === 'fr' ? fr : en;
  const hasRecord = !!t;
    ...
    {isExp && <div className="border-t border-white/10 p-3 space-y-3 bg-black/10">
      {hasRecord ? <>
        <div className="flex gap-2 flex-wrap">
          ...
          {effectiveSummary.status==='NOT_STARTED' && summary.total > 0 && onStart && <button onClick={()=>onStart(t)} className="px-3 py-1.5 rounded-lg border-2 border-emerald-400/40 bg-emerald-400/10 text-emerald-200 text-[10px] font-black shadow-[0_0_18px_rgba(52,211,153,.10)]"><PlayCircle size={11} className="inline me-1"/>{T('START FIRST MATCH','بدء أول مباراة','DÉMARRER LE PREMIER MATCH')}</button>}
          {effectiveSummary.status==='IN_PROGRESS' && onStart && <button onClick={()=>onStart(t)} className="px-3 py-1.5 rounded-lg border-2 border-amber-400/40 bg-amber-400/10 text-amber-200 text-[10px] font-black"><PlayCircle size={11} className="inline me-1"/>{T('START NEXT MATCH','بدء المباراة التالية','DÉMARRER LE MATCH SUIVANT')}</button>}
          {effectiveSummary.status==='SEMIFINAL' && onStart && <button onClick={()=>onStart(t)} className="px-3 py-1.5 rounded-lg border-2 border-fuchsia-400/45 bg-fuchsia-400/10 text-fuchsia-200 text-[10px] font-black"><PlayCircle size={11} className="inline me-1"/>{T('START SEMIFINAL','بدء نصف النهائي','DÉMARRER LA DEMI-FINALE')}</button>}
          {effectiveSummary.status==='FINAL' && onStart && <button onClick={()=>onStart(t)} className="px-3 py-1.5 rounded-lg border-2 border-violet-400/50 bg-violet-500/10 text-violet-200 text-[10px] font-black"><Crown size={11} className="inline me-1"/>{T('START FINAL','بدء النهائي','DÉMARRER LA FINALE')}</button>}
          ...
          <button onClick={()=>onViewDetails({tournamentName,gender,ageGroup,weight,record:t})} className="px-3 py-1.5 rounded-lg border-2 border-violet-400/50 bg-violet-400/10 text-violet-200 text-[10px] font-black shadow-[0_0_18px_rgba(167,139,250,.12)]"><Maximize size={11} className="inline me-1"/>{T('VIEW ALL','عرض الكل','TOUT AFFICHER')}</button>
          {effectiveSummary.status==='NOT_STARTED' && <button onClick={()=>onAutoFill?.({tournamentName,gender,ageGroup,weight,record:t})} className="px-3 py-1.5 rounded-lg bg-sky-400/10 text-sky-300 text-[10px] font-black border border-sky-400/20 shadow-[0_0_18px_rgba(51,162,255,.10)]"><Plus size={11} className="inline me-1"/>{T('ADD PLAYERS / AUTO-FILL','إضافة اللاعبين / ملء تلقائي','AJOUTER JOUEURS / AUTO')}</button>}
          ...
          {effectiveSummary.status==='IN_PROGRESS' && <span className="px-3 py-1.5 rounded-lg bg-amber-400/10 text-amber-300 text-[10px] font-black"><RotateCcw size={11} className="inline me-1"/>{T('CONTINUE THIS WEIGHT','متابعة هذا الوزن','CONTINUER CE POIDS')}</span>}
          {effectiveSummary.status==='SEMIFINAL' && <span className="px-3 py-1.5 rounded-lg bg-fuchsia-400/10 text-fuchsia-300 text-[10px] font-black"><Trophy size={11} className="inline me-1"/>{T('SEMIFINAL IN PROGRESS','نصف النهائي جارٍ','DEMI-FINALE EN COURS')}</span>}
          {effectiveSummary.status==='FINAL' && <span className="px-3 py-1.5 rounded-lg border-2 border-violet-400/45 bg-violet-500/10 text-violet-200 text-[10px] font-black shadow-[0_0_24px_rgba(167,139,250,.16)]"><Crown size={11} className="inline me-1"/>{T('FINAL IN PROGRESS','النهائي جارٍ','FINALE EN COURS')}</span>}
        ...
        <div className="rounded-xl border border-white/10 bg-black/20 p-3">
          <div className="flex items-center gap-2 text-[10px] font-black text-[hsl(var(--gold))] mb-3"><Trophy size={13}/> {T('TOURNAMENT ROADMAP','خارطة البطولة','FEUILLE DE ROUTE')}</div>
        ...
        <div className="rounded-xl border border-[hsl(var(--gold))]/20 bg-[hsl(var(--gold))]/[.035] p-3">
          <div className="flex items-center gap-2 text-[10px] font-black text-[hsl(var(--gold))] mb-3"><BarChart3 size={13}/> {T('WEIGHT STATISTICS','إحصائيات الوزن','STATISTIQUES DU POIDS')}</div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
            <div className="rounded-lg bg-black/20 p-2"><div className="text-[8px] text-white/45">{T('PLAYERS','اللاعبون','JOUEURS')}</div><div className="text-lg font-black">{summary.players}</div></div>
            <div className="rounded-lg bg-black/20 p-2"><div className="text-[8px] text-white/45">{T('MATCHES','المباريات','MATCHS')}</div><div className="text-lg font-black">{summary.total}</div></div>
            <div className="rounded-lg bg-black/20 p-2"><div className="text-[8px] text-white/45">{T('PLAYED','لُعبت','JOUÉS')}</div><div className="text-lg font-black text-emerald-300">{summary.completed}</div></div>
            <div className="rounded-lg bg-black/20 p-2"><div className="text-[8px] text-white/45">{T('REMAINING','متبقية','RESTANTS')}</div><div className="text-lg font-black text-amber-300">{summary.remaining}</div></div>
          </div>
          {(() => { const ranked:any[] = summary.ranking || []; const best=ranked[0]; const clean=[...ranked].sort((a,b)=>(a.penalties-b.penalties)||(b.wins-a.wins)||((b.points-b.scoreAgainst)-(a.points-a.scoreAgainst)))[0]; const clubs=Object.values(summary.clubStats||{}).sort((a:any,b:any)=>b.tournamentPoints-a.tournamentPoints||b.points-a.points) as any[]; const bestClub=clubs[0]; return <div className="space-y-3"><div className="grid md:grid-cols-3 gap-2">
            <div className="rounded-lg border border-[hsl(var(--gold))]/25 bg-black/20 p-3"><div className="text-[8px] text-[hsl(var(--gold))]"><Crown size={11} className="inline me-1"/>{T('BEST PLAYER','أفضل لاعب','MEILLEUR JOUEUR')}</div><div className="font-black mt-1">{best?.player||'—'}</div><div className="text-[8px] text-white/45 mt-1">W {best?.wins||0} · PTS {best?.points||0} · WR {best?.winRate||0}%</div></div>
            <div className="rounded-lg border border-emerald-400/20 bg-black/20 p-3"><div className="text-[8px] text-emerald-300"><Medal size={11} className="inline me-1"/>{T('CLEANEST PLAYER','أنظف لاعب','JOUEUR LE PLUS PROPRE')}</div><div className="font-black mt-1">{clean?.player||'—'}</div><div className="text-[8px] text-white/45 mt-1">PEN {clean?.penalties||0} · W {clean?.wins||0}</div></div>
            <div className="rounded-lg border border-violet-400/20 bg-black/20 p-3"><div className="text-[8px] text-violet-300">{T('BEST CLUB','أفضل نادٍ','MEILLEUR CLUB')}</div><div className="font-black mt-1">{bestClub?.club||'—'}</div><div className="text-[8px] text-white/45 mt-1">W {bestClub?.wins||0} · PTS {bestClub?.points||0}</div></div>
...
export default function TournamentFileCenter({ tournaments, onOpen, onStart, onAutoFill, activeMatch }: Props) {
  ...
  return <section className="panel p-4 mt-4">
    <button className="w-full flex items-center justify-between" onClick={()=>setOpen(v=>!v)}>
      <span className="flex items-center gap-2 font-display text-sm font-black text-[hsl(var(--gold))]"><FolderOpen size={16}/> {T('TOURNAMENT FILE CENTER','مركز ملفات البطولة','CENTRE DES FICHIERS DU TOURNOI')}</span>
        ...
        return <div key={tournamentName} className="rounded-2xl border border-[hsl(var(--gold))]/20 bg-black/15 overflow-hidden">
          ...
          {tournamentOpen && <div className="border-t border-white/10 p-3 space-y-3">
            ...
            {activeWeights.length>0 && <div className="rounded-xl border border-amber-400/20 bg-amber-400/[.035] p-3">
              <div className="flex items-center gap-2 text-[10px] font-black text-amber-300 mb-2"><CircleDot size={12}/> {T('CONTINUE / IN PROGRESS WEIGHTS','متابعة / أوزان قيد التنفيذ','CONTINUER / POIDS EN COURS')}</div>
          ...
          <div className="mx-auto w-full max-w-[1800px]">
            <div className="relative overflow-hidden rounded-3xl border-2 border-[hsl(var(--gold))]/35 bg-[radial-gradient(circle_at_15%_0%,rgba(242,193,78,.13),transparent_30%),radial-gradient(circle_at_90%_20%,rgba(51,162,255,.09),transparent_28%),linear-gradient(145deg,#0b0f17,#05070b)] p-5 md:p-7 shadow-[0_0_100px_rgba(242,193,78,.12)]">
              ...
              <div className="relative flex flex-wrap items-start justify-between gap-4">
                ...
                <button onClick={()=>setDetails(null)} className="rounded-xl border-2 border-white/15 bg-white/[.04] px-4 py-2.5 text-[10px] font-black text-white/80 hover:border-[hsl(var(--gold))]/50"><X size={15} className="inline me-1"/>{T('CLOSE','إغلاق','FERMER')}</button>
              </div>
            ...
            <div className="mt-4 grid grid-cols-1 xl:grid-cols-3 gap-4">
              <div className="xl:col-span-2 rounded-2xl border border-[hsl(var(--gold))]/20 bg-[#0a0d13] p-4 md:p-5">
                <div className="flex items-center gap-2 text-[11px] font-black tracking-[.18em] text-[hsl(var(--gold))]"><Trophy size={14}/> {T('TOURNAMENT ROADMAP','خارطة البطولة','FEUILLE DE ROUTE')}</div>
              ...
              <div className="rounded-2xl border border-emerald-400/20 bg-[#0a0d13] p-4 md:p-5">
                ...
                <div className="mt-4 space-y-3">
                  <div className="rounded-xl border border-[hsl(var(--gold))]/25 bg-[hsl(var(--gold))]/[.05] p-3"><div className="text-[8px] text-[hsl(var(--gold))]">{T('BEST PLAYER','أفضل لاعب','MEILLEUR JOUEUR')}</div><div className="mt-1 font-black">{ranking[0]?.player || '—'}</div><div className="mt-1 text-[8px] text-white/50">W {ranking[0]?.wins||0} · PTS {ranking[0]?.points||0} · WR {ranking[0]?.winRate||0}%</div></div>
                  <div className="rounded-xl border border-emerald-400/20 bg-emerald-400/[.03] p-3"><div className="text-[8px] text-emerald-300">{T('CLEANEST PLAYER','أنظف لاعب','JOUEUR LE PLUS PROPRE')}</div><div className="mt-1 font-black">{clean?.player || '—'}</div><div className="mt-1 text-[8px] text-white/50">PEN {clean?.penalties||0} · W {clean?.wins||0}</div></div>
                  <div className="rounded-xl border border-violet-400/20 bg-violet-400/[.03] p-3"><div className="text-[8px] text-violet-300">{T('BEST CLUB','أفضل نادٍ','MEILLEUR CLUB')}</div><div className="mt-1 font-black">{clubs[0]?.club || '—'}</div><div className="mt-1 text-[8px] text-white/50">W {clubs[0]?.wins||0} · PTS {clubs[0]?.points||0}</div></div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 rounded-2xl border border-white/10 bg-[#0a0d13] p-4 md:p-5">
              ...
              <div className="mt-3 grid grid-cols-1 xl:grid-cols-2 gap-2">
                ...
                {(!summary.matchRows || summary.matchRows.length===0) && <div className="xl:col-span-2 py-6 text-center text-[9px] text-white/35">{T('NO MATCHES SCHEDULED YET','لا توجد مباريات مجدولة بعد','AUCUN MATCH PROGRAMMÉ')}</div>}
              </div>
            </div>
