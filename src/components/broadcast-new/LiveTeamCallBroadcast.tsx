...
function buildTeam(state: AppMatchState, side: PlayerColor): Team {
  ...
  return {
    ...
    coach: state.coachNames?.[side] || (state[side].player as any).coachName || '',
    rank: '—',
    ...
    <div dir="ltr" className={`team-call-fullscreen-shell${isMiniPreview ? " team-call-mini-preview" : ""}`} data-team-call-animation="true">
      ...
      <main className="broadcast broadcast-animation-fullscreen" data-broadcast-animation="true">
        <div className="team-call-event-meta" data-wab-layer-id="team-call-event-meta">
          <div className="team-call-event-title">{state.competitionName || 'WAB-TKD CHAMPIONSHIP'}</div>
          <div className="team-call-event-info">
            {[
            ...
            ].map(([label, value]) => <div key={label} className="team-call-event-tile"><span>{label}</span><b>{value}</b></div>)}
          </div>
        </div>
        <div className="dashboard">
          ...
          <CenterMatch
            ...
            frameOffsetX={state.config.teamCallFrameOffsetX}
            frameOffsetY={state.config.teamCallFrameOffsetY}
            frameScale={state.config.teamCallFrameScale}
            frameBorderColor={state.config.teamCallFrameBorderColor}
            frameFrozen={state.config.teamCallFrameFrozen}
            titleColor={state.config.teamCallTitleColor}
          />
