...
export function TeamPanel({ team, state, nameFormat }: Props) {
  ...
  return (
    <section className={cls} aria-label={team.name} data-wab-layer-id={`${team.tone}-team-group`}>
      <div className="energy" data-wab-layer-id={`${team.tone}-glow`} />

      <header className="team-head" data-wab-layer-id={`${team.tone}-header`}>
        <div className="crest" data-wab-layer-id={`${team.tone}-logo`}>
        ...
        <div className="team-title" data-wab-layer-id={`${team.tone}-name`}>
      ...
      <div className="roster" data-wab-layer-id={`${team.tone}-roster`}>
        ...
        {team.players.map((p, i) => (
          <div className="player-row" key={`${p.name}-${i}`}>
            <span className="pnum">{p.playerNumber ?? i + 1}</span>
            ...
            <span className="pseed-rank">
              <b>{p.seed}</b>
              <span className="player-number-under-seed">PLAYER NO. {p.playerNumber ?? '—'}</span>
            </span>
      ...
      <div className="club-banner" data-wab-layer-id={`${team.tone}-club`}>
      ...
      <div
        data-wab-layer-id={`${team.tone}-status`}
