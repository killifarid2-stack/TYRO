...
import { listCategoryMatches } from '@/lib/category-library';
...
export default function OperatorScreen() {
  ...
  const [showMiniPreview, setShowMiniPreview] = useState(() => {
    // The referee's monitor is a persistent tool: opening AI, WOO-SE-GIROK,
    // replay, result screens or finishing a match must never close it.
    try { return localStorage.getItem('wab_referee_live_preview_open') !== '0'; } catch { return true; }
  });
  const [previewTab, setPreviewTab] = useState<'public' | 'judge'>('public');
  ...
  useEffect(() => {
    ...
    channel = getChannel('judge-votes');
    channel
    ...
    void connect();
    return () => { cancelled = true; if (channel) teardownChannel(channel); };
  ...
  useEffect(() => {
    ...
    if (winnerCountdown <= 0) {
      ...
      if (savedMatchId !== state.id) {
        toast.info(lang === 'ar' ? 'احفظ المباراة أولاً ثم اضغط متابعة المباراة التالية.' : lang === 'fr' ? 'Enregistrez d’abord le match, puis passez au suivant.' : 'SAVE MATCH first, then continue to the next match.');
        return;
      }
      ...
      (async () => {
        try {
          ...
          toast.error(lang === 'ar' ? 'تعذر الانتقال للمباراة التالية. المباراة محفوظة ويمكنك العودة يدوياً.' : lang === 'fr' ? 'Impossible d’ouvrir le match suivant. Le résultat est enregistré.' : 'Could not open the next match. The result is saved; you can return manually.');
        }
      })();
      return;
    }
  ...
  const handleSaveMatch = async () => {
    ...
    if (state.status === 'finished' && state.result && !state.resultConfirmed) {
      toast.error(lang === 'ar' ? 'يجب على الحكم الرئيسي تأكيد النتيجة الرسمية أولاً.' : lang === 'fr' ? 'L’arbitre principal doit d’abord confirmer le résultat officiel.' : 'MAIN REFEREE must confirm the official result first.');
      return;
    }
    ...
    if (state.config.trainingMode) {
      ...
      toast.success(lang === 'ar' ? '🎓 وضع التدريب — الماتش ماتسجلش فالأرشيف الرسمي' : lang === 'fr' ? '🎓 MODE ENTRAÎNEMENT — non enregistré dans l’archive officielle' : '🎓 TRAINING MODE — not saved to official archive');
      return;
    }
    setSavingMatch(true);
    try {
      ...
      setShowSaveConfirm(false);
      toast.success(result.alreadySaved ? (lang === 'ar' ? 'المباراة محفوظة بالفعل' : lang === 'fr' ? 'MATCH DÉJÀ ENREGISTRÉ' : 'MATCH ALREADY SAVED') : (lang === 'ar' ? '✓ تم حفظ المباراة — مكتملة' : lang === 'fr' ? '✓ MATCH ENREGISTRÉ — TERMINÉ' : '✓ MATCH SAVED — COMPLETED'));
    } catch (e: any) {
  ...
  const ivrReturnStatusRef = useRef<'fighting' | 'rest'>('fighting');

  const handleIVR = (color: PlayerColor) => {
    // Defensive guard (same pattern as substitution/team-call): don't rely
    // solely on the button's disabled={state.status==='ivr'} attribute,
    // which can lag a render behind a fast double-click — check state
    // directly so a stray second click can never overwrite an in-flight
    // request with the other side's color.
    if (state.status === 'ivr' || ivrRequester) return;
    if (state[color].ivrQuota > 0) {
      ivrReturnStatusRef.current = state.status === 'rest' ? 'rest' : 'fighting';
  ...
  const handleIVRDecision = (decision: 'accept' | 'reject') => {
    ...
    // Shows for ~3s then returns to the live match automatically — the
    // manual "HIDE RESULT" button (below) still works for an earlier
    // dismiss. This was briefly changed to stay until manually dismissed;
    // reverted back to auto-clear per explicit request.
    // Freeze the fight clock while the verdict is on screen, then return to
    // the exact phase that was interrupted. The public animation itself is
    // guaranteed to stay visible for the full 3-second decision dwell.
    dispatch({ type: 'SET_STATUS', status: 'paused' });
    setIvrRequester(null);
    window.setTimeout(() => {
      dispatch({ type: 'CLEAR_IVR_ANIMATE' });
      dispatch({ type: 'SET_STATUS', status: ivrReturnStatusRef.current });
    }, 3000);
  };
  ...
  // NEXT MATCH → RESET wipes the whole match state (fresh bout), which used
  // to also wipe the tournament/weight/age/gender info — forcing the
  // operator to retype it for every single bout even when working straight
  // through one weight category. Carry those fields over into the fresh
  // state instead; the Quick Match form that opens right after is still
  // fully editable, so picking a different match/category is one edit away,
  // not a full retype.
  const handleNextMatch = async () => {
    ...
    setSaveCompletedNotice(false);
    setWinnerCountdown(null);
    setSavedMatchId(null);

    // Find the next unplayed match in the SAME bracket the match that just
    // finished belongs to — same source of truth the audience "NEXT ON THE
    // MAT" card already reads (state.tournamentId + bracketMatchId), so the
    // two agree. When found, its real players/weight are pre-loaded (not
    // just the category) — the Quick Match form still opens right after so
    // the operator can review/edit/confirm before it goes live, never an
    // unattended auto-start.
    let nextBracketMatch: any = null;
    const tournamentId = state.tournamentId;
    const currentBracketMatchId = state.bracketMatchId;

    // ALWAYS refresh the tournament bracket first. The visible MatchStrip can
    // lag one render behind the just-saved result; using that stale array first
    // was the reason NEXT MATCH could fail to advance (and older builds opened
    // the search flow). The saved tournament bracket is the authoritative
    // sequence, then the visible strip is only a fallback.
    if (tournamentId) {
      try {
        let bracket: any[] | null = null;
        if (isLocalTournamentId(tournamentId)) {
          bracket = loadTournamentLocal(tournamentId)?.bracket_data?.bracket ?? null;
        } else {
          const { data: t } = await supabase.from('tournaments').select('bracket_data').eq('id', tournamentId).single();
          bracket = (t?.bracket_data as any)?.bracket ?? null;
        }
        const source = Array.isArray(bracket) ? bracket : [];
        const ordered = source.slice().sort((a:any,b:any) =>
          Number(a.position ?? a.matchNumber ?? 0) - Number(b.position ?? b.matchNumber ?? 0)
        );
        const currentWeight = String(state.weightCategory || '').trim().toUpperCase();
        const sameWeight = (m:any) => {
          const mw = String(m?.weightCategory || m?.weight_category || '').trim().toUpperCase();
          return !mw || !currentWeight || mw === currentWeight;
        };
        const playable = (m:any) => Boolean(m && !m.winner && !m.isBye && m.player1 && m.player2 && sameWeight(m));
        const currentIndex = currentBracketMatchId
          ? ordered.findIndex((m:any) => String(m.id) === String(currentBracketMatchId))
          : ordered.findIndex((m:any) => Number(m.matchNumber || 0) === Number(state.matchNumber || 0));

        // Prefer the bracket's explicit nextMatchId when it exists (semifinal
        // -> final, etc.), otherwise advance exactly one card in order.
        if (currentIndex >= 0) {
          const current = ordered[currentIndex];
          if (current?.nextMatchId) {
            const linked = ordered.find((m:any) => String(m.id) === String(current.nextMatchId));
            if (playable(linked)) nextBracketMatch = linked;
          }
          if (!nextBracketMatch) {
            nextBracketMatch = ordered.slice(currentIndex + 1).find(playable) || null;
          }
        } else {
          nextBracketMatch = ordered.find(playable) || null;
        }
      } catch { /* use visible strip/category continuation below */ }
    }

    // The visible MatchStrip is the second source of truth. This is important
    // for local/offline tournaments and keeps NEXT MATCH aligned with the
    // cards the referee sees above the timer.
    if (!nextBracketMatch && Array.isArray(refBracket) && refBracket.length > 0) {
      const orderedStrip = refBracket.slice().sort((a:any,b:any) =>
        Number(a.position ?? a.matchNumber ?? 0) - Number(b.position ?? b.matchNumber ?? 0)
      );
      const currentWeight = String(state.weightCategory || '').trim().toUpperCase();
      const sameWeight = (m:any) => {
      ...
      const ready = (m:any) => Boolean(m && !m.winner && !m.isBye && m.player1 && m.player2 && sameWeight(m));
      const idx = currentBracketMatchId
        ? orderedStrip.findIndex((m:any) => String(m.id) === String(currentBracketMatchId))
        : orderedStrip.findIndex((m:any) => Number(m.matchNumber || 0) === Number(state.matchNumber || 0));
      if (idx >= 0) nextBracketMatch = orderedStrip.slice(idx + 1).find(ready) || null;
      else nextBracketMatch = orderedStrip.find(ready) || null;
    }

    // Category-library tournaments intentionally use match records instead of
    // a generated bracket. In that case NEXT MATCH must still advance inside
    // the SAME gender/age/weight bucket, in match-number order, and must never
    // select a finished/live row as the next bout.
    if (!nextBracketMatch && tournamentId) {
      try {
        const rows = await listCategoryMatches(tournamentId);
        const currentNo = Number(state.matchNumber || 0);
        const nextRow = rows
          .filter((m:any) => String(m.status || '').toLowerCase() === 'waiting' && Number(m.matchNumber || 0) > currentNo)
          .sort((a:any,b:any) => Number(a.matchNumber || 0) - Number(b.matchNumber || 0))[0];
        if (nextRow) {
          nextBracketMatch = {
            id: nextRow.id,
            player1: { name: nextRow.hongName || '' },
            player2: { name: nextRow.chungName || '' },
            match_stage: nextRow.stage || state.matchStage,
            weightCategory: state.weightCategory,
          };
        }
      } catch { /* genuine fallback: manual selector */ }
    }

    dispatch({ type: 'RESET' });
    dispatch({ type: 'SET_MATCH_INFO', ...carry, ...(nextBracketMatch ? { tournamentId, bracketMatchId: nextBracketMatch.id } : {}) });
    if (nextBracketMatch) {
      dispatch({ type: 'SET_PLAYER', color: 'hong', name: nextBracketMatch.player1.name || '', nationality: nextBracketMatch.player1.nationality || '', club: nextBracketMatch.player1.club, seedNumber: nextBracketMatch.player1.seedNumber, playerNumber: nextBracketMatch.player1.playerNumber, photoUrl: nextBracketMatch.player1.photoUrl });
      dispatch({ type: 'SET_PLAYER', color: 'chung', name: nextBracketMatch.player2.name || '', nationality: nextBracketMatch.player2.nationality || '', club: nextBracketMatch.player2.club, seedNumber: nextBracketMatch.player2.seedNumber, playerNumber: nextBracketMatch.player2.playerNumber, photoUrl: nextBracketMatch.player2.photoUrl });
      // Both players are already correct (same bracket, same weight/age/
      // gender as the match that just finished) — go straight back to the
      // Operator screen with them loaded, ready to review and start.
      // Opening the search modal here was the actual bug: it discarded the
      // pre-loaded match and made the operator re-find it manually every
      // time, even though the right next match had already been found.
      return;
    }
    // There is no next playable bout in this tournament/category. Do NOT
    // open the search dialog: NEXT MATCH is a sequential command, while the
    // magnifier button beside it remains the explicit manual selector.
    toast.info(lang === 'ar'
      ? 'لا توجد مباراة تالية جاهزة في نفس البطولة والوزن.'
      : lang === 'fr'
        ? 'Aucun prochain match prêt dans le même tournoi et la même catégorie.'
        : 'No next ready match in the same tournament and weight category.');
  };
  ...
  // Persist referee name AND broadcast it into match state — previously
  // this only ever reached localStorage, so the public display's
  // "REFEREE CONNECTED" indicator could show YES/WAITING but never the
  // referee's actual name, since state.mainRefereeName never existed.
  useEffect(() => {
    localStorage.setItem('tkd-referee-name', refereeName);
    if (state.mainRefereeName !== refereeName) {
      dispatch({ type: 'SET_STATE', state: { ...state, mainRefereeName: refereeName } });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [refereeName]);
        ...
        <div className="panel p-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            ...
            <button onClick={handleOpenMatchSelector}
              className="p-1.5 rounded-lg border-2 border-[hsl(var(--gold))]/60 bg-[hsl(var(--gold))]/20 text-[hsl(var(--gold))] hover:bg-[hsl(var(--gold))]/30 transition-colors" title={t('selectMatchTitle')}>
              <Search size={14} />
            </button>
            ...
            <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/92 p-3 md:p-6 backdrop-blur-xl">
              <div className="relative w-full max-w-5xl max-h-[94vh] overflow-y-auto rounded-2xl border-2 bg-[radial-gradient(circle_at_50%_0%,rgba(255,255,255,.05),transparent_32%),#070a10] p-4 md:p-6 shadow-[0_30px_120px_rgba(0,0,0,.85)]" style={{ borderColor: `${accent}88`, boxShadow: `0 0 60px ${accent}18, 0 30px 120px rgba(0,0,0,.85)` }}>
                ...
                <div className="mt-4 flex flex-wrap justify-end gap-2">
                  ...
                  <button disabled={!subInputName.trim() || !!state.substitutionAnimation} onClick={() => { if (!state.pendingSubstitution || state.substitutionAnimation) return; const side = state.pendingSubstitution.side; confirmPlayerChange(side, subInputName.trim()); setSubInputName(''); }} className="rounded-xl px-6 py-3 text-sm font-black text-black disabled:opacity-30" style={{ background: accent }}>CONFIRM PLAYER CHANGE</button>
                </div>
              </div>
            </div>
          );
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <button onClick={() => handleAcceptJudgeScore(notif)}
                      className="p-1.5 rounded-lg border-2 border-[hsl(var(--success))]/60 bg-[hsl(var(--success))]/20 text-[hsl(var(--success))] active:scale-95">
                      <Check size={16} />
                    </button>
                    <button onClick={() => handleRejectJudgeScore(notif)}
                      className="p-1.5 rounded-lg border-2 border-[hsl(var(--destructive))]/60 bg-[hsl(var(--destructive))]/20 text-[hsl(var(--destructive))] active:scale-95">
                      <X size={16} />
                    </button>
                  </div>
                </div>
          ...
          <div className="panel p-4 border-2 border-[hsl(var(--primary))]/30 animate-fade-in space-y-3">
            ...
            <div className="rounded-lg p-3 bg-[hsl(var(--secondary))]/30 border border-[hsl(var(--gold))]/30">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  ...
                  {(Object.keys(MATCH_STAGE_LABELS) as MatchStage[]).map(stage => (
                    <option key={stage} value={stage}>{MATCH_STAGE_LABELS[stage][lang]}</option>
                  ))}
                </select>
                <input
                  type="number"
              ...
              <button
                ...
                className="w-full mt-2 px-3 py-1.5 rounded border-2 border-[hsl(var(--success))]/60 bg-[hsl(var(--success))]/20 text-[hsl(var(--success))] font-bold text-xs">
                ✓ Apply &amp; Reset Match
              </button>
            </div>
                  ...
                  {matchSelectorData.map(tn => (
                    <div key={tn.id} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[hsl(var(--secondary))]/50">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded border-2 border-[hsl(var(--destructive))]/60 bg-[hsl(var(--destructive))]/20 text-[hsl(var(--destructive))] hover:bg-[hsl(var(--destructive))]/30"
                  ...
                  {savedMatchesData.map(m => (
                    <div key={m.id} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[hsl(var(--secondary))]/50">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded border-2 border-[hsl(var(--destructive))]/60 bg-[hsl(var(--destructive))]/20 text-[hsl(var(--destructive))] hover:bg-[hsl(var(--destructive))]/30 shrink-0"
        ...
        {showKeyboardMap && (
          <div className="panel p-3 animate-fade-in">
            <div className="flex items-center gap-2 mb-2">
              ...
              <button onClick={() => setShowKeyConfig(true)} className="ms-auto text-xs px-2 py-1 rounded border-2 border-[hsl(var(--primary))]/60 bg-[hsl(var(--primary))]/20 text-[hsl(var(--primary))] font-semibold flex items-center gap-1">
            ...
            {!showKeyConfig ? (
              ...
              <div className="space-y-1 max-h-64 overflow-y-auto">
                ...
                <div className="flex gap-2 mt-2">
                  <button onClick={() => setKeyMap({ ...DEFAULT_KEY_MAP })} className="text-xs px-3 py-1 rounded border-2 border-[hsl(var(--destructive))]/60 bg-[hsl(var(--destructive))]/20 text-[hsl(var(--destructive))]">{t('resetDefaultsLabel')}</button>
                  <button onClick={() => setShowKeyConfig(false)} className="text-xs px-3 py-1 rounded border-2 border-[hsl(var(--success))]/60 bg-[hsl(var(--success))]/20 text-[hsl(var(--success))]">{t('doneLabel')}</button>
                </div>
              </div>
            )}
          </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <button onClick={() => { navigator.clipboard.writeText(judgeUrl); sounds.click(); }}
                      className="px-3 py-1.5 rounded border-2 border-[hsl(var(--primary))]/60 bg-[hsl(var(--primary))]/20 text-[hsl(var(--primary))] text-xs font-semibold">
                      {t('copy')}
                    </button>
                  </div>
                </div>
              ...
              <div className="flex flex-col md:flex-row gap-3 items-start">
                ...
                <div className="flex-1 w-full space-y-2">
                  ...
                  <button onClick={() => {
                    ...
                    className="px-3 py-1.5 rounded border-2 border-[hsl(var(--gold))]/60 bg-[hsl(var(--gold))]/20 text-[hsl(var(--gold))] text-xs font-semibold">
                ...
                <div className="flex gap-2">
                  {(window as any).electronAPI && (
                    <button onClick={async () => { await localWifi.openHotspotSettings(); sounds.click(); }}
                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg border-2 border-[hsl(var(--success))]/60 bg-[hsl(var(--success))]/20 text-[hsl(var(--success))] text-xs font-semibold">
        ...
        <div className="panel p-4 flex flex-col items-center">
          ...
          <div className="flex items-center gap-3 mt-3">
            ...
            {state.status === 'fighting' && (
              ...
              <button onClick={() => dispatch({ type: 'PAUSE' })} className="flex items-center gap-1 px-4 py-2 rounded-lg border-2 border-[hsl(var(--warning))]/60 bg-[hsl(var(--warning))]/20 text-[hsl(var(--warning))] font-semibold text-sm">
            ...
            {state.status === 'kyeshi' && (
              <div className="flex gap-2">
                <button onClick={() => handleResumeFromKyeshi(true)} className="flex items-center gap-1 px-4 py-2 rounded-lg border-2 border-[hsl(var(--success))]/60 bg-[hsl(var(--success))]/20 text-[hsl(var(--success))] font-semibold text-sm">
                  <Play size={16} /> Resume ({savedTime !== null ? formatTime(savedTime) : ''})
                </button>
                <button onClick={() => handleResumeFromKyeshi(false)} className="flex items-center gap-1 px-3 py-2 rounded-lg border-2 border-[hsl(var(--primary))]/60 bg-[hsl(var(--primary))]/20 text-[hsl(var(--primary))] font-semibold text-xs">
            ...
            <button onClick={handleKyeshi}
              className="flex items-center gap-1 px-3 py-2 rounded-lg border-2 border-[hsl(var(--destructive))]/60 bg-[hsl(var(--destructive))]/20 text-[hsl(var(--destructive))] font-semibold text-sm">
            ...
            {state.status === 'rest' && !state.roundCorrectionReview && (
              <button onClick={handleSkipRest}
                className="flex items-center gap-1 px-3 py-2 rounded-lg border-2 border-[hsl(var(--info))]/60 bg-[hsl(var(--info))]/20 text-[hsl(var(--info))] font-semibold text-sm">
              ...
              {ivrDecision === null ? (
                <div className="flex gap-2 justify-center">
                  <button onClick={() => handleIVRDecision('accept')}
                    className="px-6 py-3 rounded-lg border-2 border-[hsl(var(--success))] bg-[hsl(var(--success))]/25 text-[hsl(var(--success))] font-semibold text-lg flex items-center gap-2 shadow-[0_0_18px_hsl(var(--success)/0.25)] active:scale-95 hover:bg-[hsl(var(--success))]/35">
                    <Check size={20} /> ACCEPT
                  </button>
                  <button onClick={() => handleIVRDecision('reject')}
                    className="px-6 py-3 rounded-lg border-2 border-[hsl(var(--destructive))] bg-[hsl(var(--destructive))]/25 text-[hsl(var(--destructive))] font-semibold text-lg flex items-center gap-2 shadow-[0_0_18px_hsl(var(--destructive)/0.25)] active:scale-95 hover:bg-[hsl(var(--destructive))]/35">
                    <X size={20} /> REJECT
                  </button>
                </div>
              ) : (
                <div className="text-center">
                  <div className={`font-display text-lg font-bold ${ivrDecision === 'accept' ? 'text-[hsl(var(--success))]' : 'text-[hsl(var(--destructive))]'}`}>
                    {ivrDecision === 'accept' ? '✓ ACCEPTED' : '✗ REJECTED'}
                  </div>
                  <button onClick={() => { dispatch({ type: 'CLEAR_IVR_ANIMATE' }); setIvrDecision(null); }}
                    className="mt-2 px-4 py-1.5 rounded-lg border-2 border-white/25 bg-white/5 text-white/70 font-semibold text-xs hover:bg-white/10 active:scale-95">
                    HIDE RESULT FROM PUBLIC SCREEN
                  </button>
                </div>
              )}
            </div>
          ...
          <div className="flex gap-2 mt-2 flex-wrap justify-center">
            ...
            {state.status === 'finished' && (
              <button onClick={handleExportPDF}
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg border-2 border-[hsl(var(--gold))]/60 bg-[hsl(var(--gold))]/20 text-[hsl(var(--gold))] font-semibold text-xs">
      ...
      {showSaveConfirm && state.status === 'finished' && state.result && state.resultConfirmed && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/75 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-2xl border border-[hsl(var(--gold))]/40 bg-[hsl(var(--background))] p-5 shadow-2xl">
            <div className="text-lg font-black text-[hsl(var(--gold))]">{lang === 'ar' ? 'حفظ المباراة' : lang === 'fr' ? 'ENREGISTRER LE MATCH' : 'SAVE MATCH'}</div>
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={() => setShowSaveConfirm(false)} className="flex-1 py-2.5 rounded-lg bg-white/10 text-white font-bold">{lang === 'ar' ? 'إلغاء' : lang === 'fr' ? 'ANNULER' : 'CANCEL'}</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#03060c]/92 backdrop-blur-md p-5" style={{ animation: 'refReviewIn .35s ease-out' }}>
          ...
          <div className="w-full max-w-5xl max-h-[92vh] overflow-y-auto rounded-3xl border border-[hsl(var(--gold))]/30 bg-[linear-gradient(145deg,#0b111d,#070a11)] shadow-[0_30px_100px_rgba(0,0,0,.65),0_0_60px_rgba(242,193,78,.08)]">
            ...
            <div className="px-5 pb-5 flex flex-wrap justify-center gap-2">
              ...
              {state.config?.competitionMode === 'par_equipe' && (
                <button onClick={() => dispatch({ type: 'TOGGLE_TEAM_RESULT_CARD' })}
                  className={`px-5 py-3 rounded-xl border-2 text-xs font-black transition-colors ${state.showTeamResultCard ? 'border-[hsl(var(--gold))] bg-[hsl(var(--gold))]/20 text-[hsl(var(--gold))]' : 'border-white/20 bg-black/50 text-white/80'}`}
                  title="Toggles an alternate result card on the public screen — team logo, name and club, instead of TeamWinnerScreen's trophy layout">
                  {state.showTeamResultCard ? '◉ SHOWING TEAM RESULT CARD' : '○ SHOW TEAM RESULT CARD'}
                </button>
              )}
              {state.resultConfirmed && savedMatchId === state.id && (
                <button onClick={handleNextMatch} className="px-8 py-3 rounded-xl border-2 border-emerald-400/60 bg-emerald-400/12 text-emerald-200 font-display font-black text-sm shadow-[0_0_24px_rgba(52,211,153,.14)] hover:bg-emerald-400/20">NEXT MATCH →</button>
              )}
            </div>
          </div>
        </div>
      ...
      {saveCompletedNotice && savedMatchId === state.id && (
        <div className="fixed inset-0 z-[85] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl border-2 border-emerald-400/40 bg-[#07110d] p-6 text-center shadow-[0_30px_100px_rgba(0,0,0,.75)]">
            ...
            <div className="text-xl font-black text-emerald-300">{lang === 'ar' ? 'تم حفظ المباراة بنجاح' : lang === 'fr' ? 'MATCH ENREGISTRÉ AVEC SUCCÈS' : 'MATCH SAVED SUCCESSFULLY'}</div>
            ...
            <div className="mt-5 grid grid-cols-2 gap-2">
              ...
              <button onClick={handleNextMatch} className="rounded-xl bg-emerald-400 py-3 text-sm font-black text-black">NEXT MATCH</button>
            </div>
          </div>
        </div>
      )}
      ...
      {alertActive && (
        <div className="fixed inset-0 bg-[hsl(var(--destructive))]/20 backdrop-blur-sm flex items-center justify-center z-40 animate-fade-in">
          <div className="text-center p-8 panel">
            ...
            <button onClick={() => setAlertActive(false)} className="px-6 py-2 rounded-lg border-2 border-[hsl(var(--warning))]/60 bg-[hsl(var(--warning))]/20 text-[hsl(var(--warning))] font-bold">OK</button>
          </div>
        </div>
      )}

      {/* ===== PERSISTENT FLOATING REFEREE MONITOR =====
          This is a real, read-only mirror of the Public/Judge screen. It is
          deliberately rendered through a portal at document.body level so
          parent panels/modals/fullscreen containers cannot clip or pin it to
          the bottom. The referee can drag it anywhere and resize it. It is
          never closed by match-end, AI analysis, WOO-SE-GIROK, replay,
          result, substitution, or other operator panels — only the X button
          closes it. */}
      {showMiniPreview && typeof document !== 'undefined' && createPortal(
        <div
        ...
        >
          <div
            onMouseDown={(e) => {
              ...
              const onMove = (ev: MouseEvent) => {
                ...
                // Keep the whole window reachable, but do not force it back to
                // the bottom/right. It can be placed anywhere on the desktop.
                const w = previewWidth;
                const h = w * 9 / 16 + 54;
                const x = Math.max(0, Math.min(Math.max(0, window.innerWidth - w), origX + (ev.clientX - startX)));
                const y = Math.max(0, Math.min(Math.max(0, window.innerHeight - h), origY + (ev.clientY - startY)));
                setPreviewPos({ x, y });
              };
              const onUp = () => {
            ...
            title="اسحب النافذة بحرية إلى أي مكان"
          >
            <span className="text-[10px] font-display font-bold text-[hsl(var(--info))] tracking-wider flex items-center gap-1">
              <Eye size={11} /> LIVE PREVIEW — {previewTab === 'public' ? 'PUBLIC SCREEN' : 'JUDGE SCREEN'} · READ ONLY · PERSISTENT
            </span>
            <button onClick={() => { setShowMiniPreview(false); try { localStorage.setItem('wab_referee_live_preview_open', '0'); } catch {} }} className="text-[hsl(var(--info))]/70 hover:text-[hsl(var(--info))] p-0.5" title="إغلاق نافذة شاشة الجمهور">
              <X size={12} />
            </button>
          </div>
          <div className="flex items-center gap-1 px-2 py-1 bg-black/40 border-b border-[hsl(var(--info))]/15">
            <button onClick={() => setPreviewTab('public')} className={`px-2 py-0.5 rounded text-[9px] font-display font-black tracking-wider ${previewTab === 'public' ? 'bg-[hsl(var(--info))]/25 text-[hsl(var(--info))]' : 'text-white/40 hover:text-white/70'}`}>PUBLIC SCREEN</button>
            <button onClick={() => setPreviewTab('judge')} className={`px-2 py-0.5 rounded text-[9px] font-display font-black tracking-wider ${previewTab === 'judge' ? 'bg-[hsl(var(--info))]/25 text-[hsl(var(--info))]' : 'text-white/40 hover:text-white/70'}`}>JUDGE SCREEN</button>
          </div>
          <div style={{ width: previewWidth, height: previewWidth * 9 / 16, overflow: 'hidden', position: 'relative', pointerEvents: 'none' }}>
            <div style={{ width: 1920, height: 1080, transform: `scale(${previewWidth / 1920})`, transformOrigin: 'top left' }}>
              {previewTab === 'public' ? <PublicScoreboard isMiniPreview /> : <JudgePage />}
            </div>
          </div>
          <div
            onMouseDown={(e) => {
            ...
            className="absolute bottom-0 right-0 w-5 h-5 cursor-nwse-resize flex items-end justify-end pointer-events-auto bg-black/20"
            title="اسحب لتغيير الحجم"
            style={{ insetInlineEnd: 0 }}
          >
            <svg width="11" height="11" viewBox="0 0 10 10" className="text-[hsl(var(--info))]/80 m-0.5">
