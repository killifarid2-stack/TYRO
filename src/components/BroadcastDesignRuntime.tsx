...
function applyLayer(layer: DesignLayer, runtimeTime: number, allLayers: DesignLayer[], state?: any) {
  ...
  for (const node of nodes) {
    ...
    touched.add(node);
    touched.add(node);
    let dynamic = dynamicBases.get(evaluated.id);
    // If a native layer has not been captured yet, establish its base from the
    // current rendered geometry. Never use hard-coded pixel fallbacks for a
    // real DOM element: those values were the source of resolution-dependent
    // jumps between the 1920x1080 preview and a second display.
    if (!dynamic) {
      const rr = node.getBoundingClientRect();
      if (rr.width > 0 && rr.height > 0) {
        dynamic = {
          x: rr.left / Math.max(1, window.innerWidth) * 1920,
          y: rr.top / Math.max(1, window.innerHeight) * 1080,
          width: rr.width / Math.max(1, window.innerWidth) * 1920,
          height: rr.height / Math.max(1, window.innerHeight) * 1080,
        };
        dynamicBases.set(evaluated.id, dynamic);
      }
    }
    const base = dynamic || BASE[evaluated.id] || { x: 0, y: 0, width: evaluated.width || 1, height: evaluated.height || 1 };
    // A broadcast design is authored once at 1920x1080. Fit it uniformly to
    // every 16:9 output so X/Y, frames, fonts and artwork keep the same visual
    // relationship on 1920x1080, 1280x720 and other output sizes.
    const fitScale = Math.min(window.innerWidth / 1920, window.innerHeight / 1080);
    const dx = (evaluated.x - base.x) * fitScale;
    const dy = (evaluated.y - base.y) * fitScale;
    ...
    if (evaluated.fontSize) {
      ...
      node.style.fontSize = `${autoSize * fitScale}px`;
    }
    ...
    if (evaluated.borderWidth != null) node.style.borderWidth = `${evaluated.borderWidth * fitScale}px`;
    if (evaluated.borderRadius != null) node.style.borderRadius = `${evaluated.borderRadius * fitScale}px`;
    ...
    if (evaluated.letterSpacing != null) node.style.letterSpacing = `${evaluated.letterSpacing * fitScale}px`;
