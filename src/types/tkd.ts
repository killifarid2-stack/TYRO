...
export interface Player {
  ...
  clubLogo?: string;
  /** Par Équipe only: coach assigned to this team. */
  coachName?: string;
}
...
export interface MatchConfig {
  ...
  // Lets the operator move/resize/recolor the big center trophy frame on
  // the Team Call broadcast without editing code. Values are relative
  // offsets/multipliers applied on top of the normal centered layout, so
  // leaving them at defaults (0, 0, 1, unset) reproduces the exact
  // original look — nothing changes unless the operator touches a control.
  teamCallFrameOffsetX?: number; // px, +right / -left
  teamCallFrameOffsetY?: number; // px, +down / -up
  teamCallFrameScale?: number; // multiplier, default 1
  teamCallFrameBorderColor?: string; // CSS color, overrides the default gold border
  teamCallFrameFrozen?: boolean; // pauses every CSS animation inside the frame (float, glow…) without removing it
  teamCallTitleColor?: string; // CSS color, overrides the call-title text color
}
...
export const MATCH_STAGE_LABELS: Record<MatchStage, { en: string; ar: string; fr: string }> = {
  qualification: { en: 'Qualification', ar: 'إقصائيات', fr: 'Qualifications' },
  round_of_32: { en: 'Round of 32', ar: 'دور الـ32', fr: 'Seizièmes de finale' },
  round_of_16: { en: 'Round of 16', ar: 'دور الـ16', fr: 'Huitièmes de finale' },
  quarterfinal: { en: 'Quarterfinal', ar: 'ربع النهائي', fr: 'Quarts de finale' },
  semifinal: { en: 'Semifinal', ar: 'نصف النهائي', fr: 'Demi-finales' },
  bronze: { en: 'Bronze Medal Match', ar: 'مباراة تحديد المركز الثالث', fr: 'Match pour la médaille de bronze' },
  final: { en: 'Final', ar: 'النهائي', fr: 'Finale' },
};
...
export const COMPETITION_MODE_LABELS: Record<'knockout' | 'friendly' | 'league' | 'par_equipe', { en: string; ar: string; fr: string }> = {
  knockout: { en: 'Tournament', ar: 'بطولة', fr: 'Tournoi' },
  league: { en: 'League', ar: 'دوري', fr: 'Ligue' },
  friendly: { en: 'Friendly', ar: 'ودية', fr: 'Amical' },
  par_equipe: { en: 'Par Équipe', ar: 'بار إيكيب', fr: 'Par Équipe' },
};
...
export interface BroadcastAnimationController {
  ...
  animationId?: string;
  /** Unique lifecycle instance; unlike animationId it is never reused by a replay. */
  instanceId?: string;
  commandId?: string;
  lastCommand?: string;
  startedAt?: number;
...
export interface MatchState {
  ...
  // Par Équipe match end: an alternate result card the Main Referee can
  // reveal on top of TeamWinnerScreen (which stays untouched) — same
  // medal-card layout as the individual match result, but with the TEAM
  // logo/name/club instead of a player photo. Off by default; toggled by
  // a dedicated button once the team match has ended.
  showTeamResultCard?: boolean;
  ...
  mainRefereeName?: string; // the name the main referee typed into the Operator screen — broadcast so the public display can show WHO is connected, not just a yes/no dot
