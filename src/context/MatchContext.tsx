...
type MatchAction =
  ...
  | { type: 'AUTO_SHOW_MATCHUP_AFTER_DELAY'; animationId: string };
...
function makeAnimationController(
  ...
  const now = Date.now();
  const previous = state.animationController;
  const sameLifecycle = patch.state !== 'IDLE'
    && previous?.state !== 'IDLE'
    && previous?.activeAnimation === patch.activeAnimation
    && !!previous?.instanceId;
  return {
    ...(previous || {}),
    ...patch,
    // One instanceId represents one real animation lifecycle. Repeated control
    // commands inside that lifecycle must not create a new identity, otherwise
    // late timers/listeners can no longer be reliably rejected. A genuinely new
    // animation/state gets a fresh instance.
    instanceId: patch.state === 'IDLE' ? undefined : (sameLifecycle ? previous?.instanceId : `inst-${now}-${Math.random().toString(36).slice(2, 8)}`),
...
export function matchReducer(state: MatchState, action: MatchAction): MatchState {
  switch (action.type) {
    ...
    case 'SET_MATCH_INFO': {
      ...
      if (action.bracketMatchId !== undefined) {
        newState.bracketMatchId = action.bracketMatchId;
        // A bracket/category match already has a durable identity. Reusing it
        // here makes FINISH/SAVE update the exact waiting record instead of
        // creating a second random match record, which previously left a
        // completed match playable again in the category archive.
        newState.id = action.bracketMatchId;
      }
    ...
    case 'ADVANCE_CALL_ANIMATION': {
      ...
      if (ns.callAnimation.phase === 'team') {
        ...
        ns.animationController = makeAnimationController(ns, {
          state: 'CALLING_PLAYER',
          activeAnimation: 'SINGLE_PLAYER_CALL',
          animationId: `player-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,
        }, 'ADVANCE_TEAM_TO_PLAYER:inter-round');
      }
      return ns;
    }
    ...
    case 'TOGGLE_TEAM_RESULT_CARD':
      return { ...state, showTeamResultCard: !state.showTeamResultCard };
    case 'KO_ANIMATE':
    ...
    case 'CALL_SINGLE_PLAYER': {
      ...
      return {
        ...
        // A stale 'called' from the Team Call that already finished must
        // never leak into a Player Call — see reducer comment above.
        teamCallStatus: { chung: 'idle' as const, hong: 'idle' as const },
    ...
    case 'START_PLAYER_CALL_SEQUENCE': {
      ...
      return {
        ...
        matchupAnimation: undefined,
        callScreenActive: true,
        // Same stale-flag fix as CALL_SINGLE_PLAYER — a leftover 'called'
        // from the previous Team Call must not make the public display's
        // Team Call overlay reappear during this Player Call.
        teamCallStatus: { chung: 'idle' as const, hong: 'idle' as const },
        selectedCallPlayers: { ...(state.selectedCallPlayers || {}), chung: selection },
        playerCallStatus: { ...state.playerCallStatus, chung: 'calling' as const },
        singlePlayerCall: {
    ...
    case 'AUTO_SHOW_MATCHUP_AFTER_DELAY': {
      const seq = state.autoCallSequence;
      // A delayed callback belongs to exactly one animation instance. If a
      // newer animation replaced it before the timer fired, ignore the stale
      // callback instead of resurrecting the old Matchup/Team Call.
      if (!seq?.active || seq.stage !== 'MATCHUP_DELAY') return state;
      if (state.animationController?.animationId && action.animationId !== state.animationController.animationId) return state;
    ...
    case 'CONFIRM_SUBSTITUTION': {
      ...
      if (state.teamMode !== 'substitution') return state;
      // Defensive guard: ignore a second CONFIRM_SUBSTITUTION while the
      // player-change cinematic is still playing. The UI already disables
      // both confirm paths (button + Enter key) during playback, but the
      // reducer enforces it too so no future button/shortcut can restart
      // the animation mid-play.
      if (state.substitutionAnimation) return state;
...
export function MatchProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const animationId = state.animationController?.animationId;
    if (!animationId) return;
    const t = setTimeout(() => dispatch({ type: 'COMPLETE_TEAM_CALL', side: 'chung', animationId }), Math.max(0, msLeft));
    return () => clearTimeout(t);
  ...
  useEffect(() => {
    ...
    const animationId = state.animationController?.animationId;
    if (!animationId) return;
    const t = setTimeout(() => dispatch({ type: 'COMPLETE_TEAM_CALL', side: 'hong', animationId }), Math.max(0, msLeft));
    return () => clearTimeout(t);
  }, [isPublicWindow, state.autoCallSequence?.active, state.autoCallSequence?.stage, state.autoCallSequence?.stageEndsAt]);
  ...
  useEffect(() => {
    ...
    const animationId = state.animationController?.animationId;
    if (!animationId) return;
    const t = setTimeout(() => dispatch({ type: 'AUTO_SHOW_MATCHUP_AFTER_DELAY', animationId }), Math.max(0, msLeft));
    return () => clearTimeout(t);
  }, [isPublicWindow, state.autoCallSequence?.active, state.autoCallSequence?.stage, state.autoCallSequence?.stageEndsAt]);

  // ---- Player-change (substitution) cinematic: operator-side expiry ----
  // The public/broadcast window is a pure mirror — its own CLEAR_SUBSTITUTION_
  // ANIMATION dispatch (fired when the cinematic finishes playing locally)
  // only clears ITS local copy. The next state sync from the operator then
  // overwrites that with the still-set canonical value, making the whole
  // cinematic replay from the start — and since a sync happens on basically
  // any operator action, this looked like "the animation restarts on every
  // button press". Clearing it here too, from the operator's own timer, is
  // the same pattern already used above for every other timed cinematic.
  useEffect(() => {
    if (isPublicWindow) return;
    const sub = state.substitutionAnimation;
    if (!sub) return;
    const tagSeconds = Math.max(1, state.config?.parEquipeTagSeconds ?? 5);
    // +600ms margin so the canonical state never expires before the public
    // window's own local animation has actually finished playing.
    const msLeft = (sub.ts + tagSeconds * 1000 + 600) - Date.now();
    const t = setTimeout(() => dispatch({ type: 'CLEAR_SUBSTITUTION_ANIMATION' }), Math.max(0, msLeft));
    return () => clearTimeout(t);
  }, [isPublicWindow, state.substitutionAnimation?.ts, state.config?.parEquipeTagSeconds]);
