...
function ensureDevBuild() {
  ...
  const spawnCommand = process.platform === 'win32' ? (process.env.ComSpec || 'cmd.exe') : 'npm';
...
ipcMain.on('broadcast-design-sync', (event, design) => {
  ...
  for (const win of publicWins.values()) {
    if (win && !win.isDestroyed()) win.webContents.send('broadcast-design-sync', lastBroadcastDesign);
  }
});
ipcMain.handle('broadcast-design:get', (event) => {
  requireTrustedRenderer(event);
  return lastBroadcastDesign;
});
...
function startLocalWifiServer() {
  ...
  const wifiHeartbeatInterval = setInterval(() => {
    ...
    localWifiServer.clients.forEach((ws) => {
      if (ws.isAlive === false) {
        // Give a socket that missed one ping cycle a second chance before
        // terminating it — a single dropped pong on a weak/crowded venue
        // Wi-Fi network is common and does not mean the phone is actually
        // gone. Only terminate after two consecutive misses (~40s of real
        // silence), which still comfortably beats router/OS idle timeouts
        // while no longer flickering connect/disconnect on every minor
        // Wi-Fi hiccup.
