...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // ---- Broadcast Design Studio sync (designer -> every audience display) ----
  broadcastDesign: (design) => ipcRenderer.send('broadcast-design-sync', design),
  getBroadcastDesign: () => ipcRenderer.invoke('broadcast-design:get'),
  onBroadcastDesignSync: (callback) => {
    const listener = (_event, design) => callback(design);
    ipcRenderer.on('broadcast-design-sync', listener);
    return () => ipcRenderer.removeListener('broadcast-design-sync', listener);
  },
